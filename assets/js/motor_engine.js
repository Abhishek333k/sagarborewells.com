...
// We use a getter to safely access DOM elements even if script loads early
const UI = {
    ...
    get phase() { return document.getElementById('inp-phase'); },
    get depth() { return document.getElementById('inp-depth'); },
    get suction() { return document.getElementById('inp-suction'); },
    get delivery() { return document.getElementById('inp-delivery'); },
    get flat() { return document.getElementById('inp-flat'); },
    get lift() { return document.getElementById('inp-lift'); }
};
...
const Terminal = {
    ...
    show: () => { 
        if (UI.terminal) UI.terminal.style.display = 'flex'; 
    },
    hide: () => { 
        if (UI.terminal) UI.terminal.style.display = 'none'; 
    }
};
...
window.runMotorEngine = async function() {
    ...
    // We use safe parsing (|| 0) to prevent NaN errors
    ...
    try {
        ...
        // 4. DATA AGGREGATION (With Timeout Protection)
        ...
        // STEP 2: Smart Pre-Sorting (Bubble up best matches)
        candidates.sort((a, b) => {
            let scoreA = 0;
            let scoreB = 0;
        ...
        if(UI.debug) UI.debug.innerHTML = `TDH: <strong>${Math.round(head)} ft</strong> • ${s.phase} Phase`;
        ...
        // Don't leave user stuck in terminal, force close after 3s
        setTimeout(() => { 
            Terminal.hide(); 
...
async function askGemini(apiKey, userSpecs, candidates) {
    ...
    try {
        ...
        // Fallback: Return top 15 pre-sorted matches
